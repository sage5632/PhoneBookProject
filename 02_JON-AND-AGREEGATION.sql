...
-- AGGREGATION
--------------
-- 여러 행을 입력으로 데이터를 집계하여 하나의 행으로 반환한다
...
-- 집계 함수 수행시 null(결측치) 값을 처리할 방식을 정책으로 결정하고 수행
...
SELECT department_id, AVG(Salary)
FROM employees
WHERE AVG(Salary) >= 7000 -- WHERE 절은 GROYP BY 가 집계가 일어나기 이전에 조건 체크
GROUP BY department_id;

-- 집계 함수 실행 이전에 WHERE 절의 조건을 검사
-- 집계 함수 커럼은 WHERE 절에서 사용할 수 없다
...
-- 다중행 서브쿼리
-- 서브쿼리의 결과 레코드가 둘 이상인 것 -> 단순 비교 여산자 수행 불가
...
-- TOP-K Query
-------------------
-- rownum : 쿼리 질의 수행결과에 의한 가상의 컬럼 -> 쿼리 결과의 순서 변환
...
SELECT first_name, salary, hire_date -- 2005년 1월 1일 이전 입사자 (24)
FROM employees
WHERE salary > 12000; -- 12000 초과한 급여 받는사원 (8)
...
SELECT first_name, salary,
    ...
    DENSE_RANK() OVER (ORDER BY salary DESC) as "DENSE RANK", --  -- 중복 순위 관계 없이 차례대로 출력
