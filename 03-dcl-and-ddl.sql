...
-- 사용자 생성
CREATE USER C##PARKJH IDENTIFIED BY 1234;
-- 비밀번호 변경
ALTER USER C##PARKJH IDENTIFIED BY test;
-- 사용자 삭제
DROP USER C##PARKJH;
-- 경우에 따라 내부에 테이블 등 데이터베이스 객체가 생성된 사용자
DROP USER C##PARKJH CASCADE; -- 폭포수

-- 다시 사용자 만들기
CREATE USER C##PARKJH IDENTIFIED BY 1234;
-- sqlPlus로 접속 시도

-- 사용자 생성,권한 부여되지 않으면 아무 일도 할수 없다
...
-- 사용자 계쩡에게 접속 권한 부여
GRANT create session TO C##PARKJH;
-- 일반적으로 데이터베이스 접속, 테이블 만들어 사용하려면
-- CONNECT, RESOURCE 롤을 부여
GRANT connect, resource TO C##PARKJH;
-- Oracle 12이사에서는 사용자 테이블 스페이스에 공간 부여 필요
ALTER USER C##PARKJH DEFAULT TABLESPACE USERS QUOTA unlimited ON USERS;
...
GRANT select ON hr.employees TO C##PARKJH;
GRANT select ON hr.departments TO C##PARKJH;
...
REVOKE select ON hr.departments FROM C##PARKJH;
...
-- author.author_id를 참조하기 위한 author_id 컬럼ㅇ르 테이블에 추가
...
-- book.author_id -> author.author_id를 참조 하도록 변경(PK)
ALTER TABLE book
ADD CONSTRAINT
    fk_author_id FOREIGN KEY(author_id)
                REFERENCES author(author_id);
-- book 테이블의 author_id 컬럼에
        -- author테이블의 author_id(PK)를 참조하는 외래 키 (PK) 추가
DESC book;
